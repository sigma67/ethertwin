  ...
  <div id="app">
    <nav class="navbar navbar-expand-md navbar-light bg-light">
      <div class="container">
        ...
        <div id="navbar" class="collapse navbar-collapse">
          <ul class="navbar-nav">
            ...
            <template v-if="selectedTwin !== 0">
              ...
              <li class="nav-item">
                <router-link :to="{ name: 'documents', params: { twin: selectedTwin  }}" class="nav-link">Documents
                </router-link>
              </li>
              <li class="nav-item" v-if="isNotDistributor">
                <router-link :to="{ name: 'sensors', params: { twin: selectedTwin  }}" class="nav-link">Sensors
                </router-link>
              </li>
              <li class="nav-item">
                <router-link :to="{ name: 'sources', params: { twin: selectedTwin  }}" class="nav-link">Data sources
                </router-link>
              </li>
              <!--<li class="nav-item">
            ...
            <template v-if="isOwner">
              <li class="nav-item">
                <router-link :to="{ name: 'roles', params: { twin: selectedTwin  }}" class="nav-link">Users
                </router-link>
              </li>
            </template>
          </ul>
        ...
        <div class="nav-item">
          <button type="button" id="icon" class="btn btn-secondary" data-toggle="tooltip" data-placement="bottom"
                  :title="account" @click="$router.push('/account')">
          </button>
        </div>
      </div>
    </nav>
