        ...
        <table class="table">
          ...
          <tr v-for="(twin, i) in twins" v-if="twins.length > 0">
            ...
            <td>
              ...
              <button class="acticon" v-on:click="shareTwin(twin.address, twin.deviceId, i)">
                ...
                vm.parseAML(deviceId, twinIndex).then(
                    () => {
                        ...
                        for (let i = 0; i < self.twins[twinIndex].components.length; i++) {
                            options += '<input type="checkbox" id="component' + i + '"value="' + self.twins[twinIndex].components[i].hash + '">' + self.twins[twinIndex].components[i].name + '</input>';
                        }
                        vm.$swal({
                            ...
                            html:
                                `<p>You can allow another account to participate in the life cycle of this device. Specify the account and its role to grant it access.</p></br>
                                 ...
                                 <h5>Attributes</h5>
                                 <div id="swal-input3" class="swal2-input">${options}</div>`
                        }).then(
                            function (result) {
                                if (result.value) { // function when confirm button clicked
                                    ...
                                    for (let i = 0; i < document.getElementById("swal-input3").children.length; i++) {
                                        if (document.getElementById("swal-input3").children[i].checked == true) attributes.push(document.getElementById("swal-input3").children[i].value); //hash of component is attribute in authorization contract
                                    }
                                    let attributeTest = web3.utils.hexToBytes(attributes[0]);
                                    console.log("attributes: "+attributes + "test attribute: "+attributeTest);
                                    ...
                                    Promise.all([
                                        ...
                                        self.contracts.Authorization.addAttribute(address,
                                            attributeTest,
                                            deviceAddress,
                                            {
                                                from: vm.account
